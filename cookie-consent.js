...
const config = {
    ...
    analytics: {
        ...
        passwordProtected: true, // Enable password protection for analytics
        password: "admin123", // Default password (should be changed per site)
        passwordCookieName: "consent_admin_auth", // Cookie name for storing auth
        passwordCookieDuration: 365 // Days to remember password
    },
    
    // Language configuration
    language: {
        defaultLanguage: 'en', // Default language for the banner
        availableLanguages: [], // Empty array = all languages available
        showLanguageSelector: true, // Show language dropdown
        autoDetectLanguage: true // Auto-detect language based on country
    },
    ...
    // UI Settings
    ui: {
        modernDesign: true, // Use modern design (false for classic design)
        floatingButtonsPosition: 'left', // 'left' or 'right'
        combineFloatingButtons: true // Combine settings and admin buttons
    }
};
...
// Complete EU language translations (same as before, but we'll filter it based on config)
const allTranslations = {
    en: { /* English translation */ },
    fr: { /* French translation */ },
    de: { /* German translation */ },
    // ... all other translations same as before ...
};

// Filter translations based on config
const translations = config.language.availableLanguages.length > 0 
    ? Object.fromEntries(
        Object.entries(allTranslations).filter(([code]) => 
            config.language.availableLanguages.includes(code)
        )
      )
    : allTranslations;
...
function detectUserLanguage(geoData) {
    ...
    // Then try to get language from country if auto-detect is enabled
    if (config.language.autoDetectLanguage && geoData && geoData.country) {
    ...
    // Fallback to configured default language or browser language
    if (translations[config.language.defaultLanguage]) {
...
// Show password prompt for analytics dashboard
function showPasswordPrompt() {
    const lang = translations[detectUserLanguage({})] || translations.en;
    
    const passwordHTML = `
    <div id="cookiePasswordModal" class="cookie-password-modal">
        <div class="cookie-password-content">
            <div class="cookie-password-header">
                <h3>${lang.statsTitle}</h3>
                <span class="close-password-modal">&times;</span>
            </div>
            <div class="cookie-password-body">
                <p>${lang.statsTitle} ${lang.statsTitle.toLowerCase().includes('statistics') ? 'are' : 'is'} password protected.</p>
                <input type="password" id="adminPasswordInput" placeholder="Enter admin password">
                <button id="submitAdminPassword" class="cookie-btn save-btn">Submit</button>
            </div>
        </div>
    </div>
    
    <style>
    .cookie-password-modal {
        display: flex;
        position: fixed;
        top: 0;
        left: 0;
    ...
    .cookie-password-modal.show {
        opacity: 1;
    }
    
    .cookie-password-content {
    ...
    .cookie-password-body {
        margin-bottom: 15px;
    }
    
    .cookie-password-body p {
        margin-bottom: 15px;
        color: ${colorScheme.textDark};
    }
    
    .cookie-password-body input {
        width: 100%;
        padding: 10px;
        margin-bottom: 15px;
    ...
    .cookie-password-body input:focus {
        outline: none;
    ...
    const modal = document.getElementById('cookiePasswordModal');
    setTimeout(() => {
    ...
    document.getElementById('submitAdminPassword').addEventListener('click', function() {
        ...
        if (password === config.analytics.password) {
            // Password correct - set cookie and show dashboard
            setCookie(config.analytics.passwordCookieName, 'true', config.analytics.passwordCookieDuration);
            hidePasswordPrompt();
...
function isAdminAuthenticated() {
    if (!config.analytics.passwordProtected) return true;
    return getCookie(config.analytics.passwordCookieName) === 'true';
}
...
function injectConsentHTML(detectedCookies, language = 'en') {
    ...
    const languageSelector = config.language.showLanguageSelector ? `
    ...
    // Combined floating button if enabled
    const combinedFloatingButton = config.ui.combineFloatingButtons && config.analytics.enabled && config.analytics.showInDashboard ? `
    <div id="cookieCombinedButton" class="cookie-combined-button" title="${lang.title}">
        <div class="cookie-combined-icon">
            ...
            <span class="cookie-combined-badge">A</span>
        </div>
    </div>` : '';
    
    // Regular floating button if not combined
    const regularFloatingButton = !config.ui.combineFloatingButtons ? `
    ...
    const html = `
    <!-- Main Consent Banner -->
    <div id="cookieConsentBanner" class="cookie-consent-banner ${config.ui.modernDesign ? 'modern-design' : 'classic-design'}">
    ...
    ${config.ui.combineFloatingButtons ? combinedFloatingButton : regularFloatingButton}
    
    ${!config.ui.combineFloatingButtons ? adminButton : ''}
    ...
    <style>
    /* Main Banner Styles - Modern Design */
    .cookie-consent-banner {
        position: fixed;
        bottom: 20px;
        ${config.ui.floatingButtonsPosition === 'left' ? 'left: 20px' : 'right: 20px'};
        width: 440px;
        background: #ffffff;
        border-radius: 12px;
    ...
    /* Classic Design */
    .cookie-consent-banner.classic-design {
        width: 100%;
        bottom: 0;
        left: 0;
        right: 0;
    ...
    .cookie-consent-banner.classic-design .cookie-consent-container {
        max-width: 1200px;
        margin: 0 auto;
        display: flex;
        align-items: center;
    ...
    .cookie-consent-banner.classic-design .cookie-consent-content {
        flex: 1;
        margin-right: 20px;
    }

    .cookie-consent-banner.classic-design .cookie-consent-buttons {
        display: flex;
        gap: 10px;
        margin-top: 0;
    }

    .cookie-consent-banner.classic-design .cookie-btn {
        padding: 8px 16px;
    }
    ...
    .cookie-consent-banner.modern-design .cookie-consent-container {
        display: block;
    }

    .cookie-consent-banner.modern-design .cookie-consent-buttons {
        margin-top: 8px;
    }
    ...
    .language-selector {
        position: absolute;
        top: 15px;
        ${config.ui.floatingButtonsPosition === 'left' ? 'right: 15px' : 'left: 15px'};
    }
    ...
    .cookie-settings-button {
        position: fixed;
        bottom: 30px;
        ${config.ui.floatingButtonsPosition === 'left' ? 'left: 30px' : 'right: 30px'};
        width: 60px;
        height: 60px;
    ...
    /* Combined Floating Button */
    .cookie-combined-button {
        position: fixed;
        bottom: 30px;
        ${config.ui.floatingButtonsPosition === 'left' ? 'left: 30px' : 'right: 30px'};
        width: 60px;
    ...
    .cookie-combined-button.show {
        opacity: 1;
        transform: translateY(0);
    }

    .cookie-combined-button:hover {
        background-color: #27ae60;
        transform: translateY(-3px) scale(1.05);
        box-shadow: 0 8px 25px rgba(0, 0, 0, 0.3);
    }

    .cookie-combined-icon {
        position: relative;
    }

    .cookie-combined-icon svg {
        width: 28px;
        height: 28px;
        fill: white;
        transition: transform 0.3s ease;
    ...
    .cookie-combined-button:hover svg {
        transform: rotate(15deg);
    }
    ...
    @media (max-width: 768px) {
        .cookie-consent-banner {
            width: 90%;
            ${config.ui.floatingButtonsPosition === 'left' ? 'left: 5%' : 'right: 5%'};
            bottom: 10px;
            padding: 20px;
        }
    ...
    @media (max-width: 480px) {
        .cookie-consent-banner {
            padding: 15px;
            width: calc(100% - 30px);
            ${config.ui.floatingButtonsPosition === 'left' ? 'left: 15px' : 'right: 15px'};
        }
...
function initializeCookieConsent(detectedCookies, language) {
    ...
    // Setup language selector change event if enabled
    if (config.language.showLanguageSelector) {
    ...
    if (config.analytics.enabled && config.analytics.showInDashboard) {
        if (config.ui.combineFloatingButtons) {
            const combinedButton = document.getElementById('cookieCombinedButton');
            if (combinedButton) {
                combinedButton.addEventListener('click', function(e) {
                    // Check if click was on the badge
                    if (e.target.classList.contains('cookie-combined-badge') {
                        showAdminDashboard();
                    } else {
                ...
                setTimeout(() => {
                    combinedButton.style.display = 'flex';
                    combinedButton.classList.add('show');
                }, 100);
            }
        } else {
...
function showAdminDashboard() {
    if (config.analytics.passwordProtected && !isAdminAuthenticated()) {
        showPasswordPrompt();
    } else {
        showAnalyticsDashboard();
    }
...
function showFloatingButton() {
    ...
    if (config.ui.combineFloatingButtons) {
        const button = document.getElementById('cookieCombinedButton');
        if (button) {
